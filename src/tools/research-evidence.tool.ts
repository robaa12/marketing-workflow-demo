...
type EvidenceCitation = z.infer<typeof EvidenceCitationSchema>;
...
export const researchEvidenceTool = createTool({
  ...
  execute: async ({ query, maxResults, allowedDomains, topic }, { abortSignal }) => {
    if (abortSignal?.aborted) throw new Error('Research evidence request was cancelled.');
    const cacheKey = JSON.stringify({ query, maxResults, allowedDomains: [...allowedDomains].sort(), topic });
    const cached = evidenceCache.get(cacheKey);
    if (cached && cached.expiresAt > Date.now()) {
      return { query, cached: true, citations: cached.citations };
    }
    ...
    return { query, cached: false, citations };
  },
});
