...
import {
  runBuyerJourney,
} from '../../agents/buyer-journey/agent.js';
...
const SectionSchema = z.enum(['personas', 'buyerJourney', 'smartObjectives']);
...
export type StrategySectionRevisionInput = z.infer<
  typeof StrategySectionRevisionInputSchema
>;
...
/**
 * Regenerates exactly one reviewer-selected strategy artifact. Upstream facts
 * and the non-selected artifacts are carried through unchanged, so users do
 * not pay for a full strategy rerun when refining one decision.
 */
export function buildStrategySectionRevisionWorkflow(
  ...
  const reviseSection = createStep({
    ...
    description: 'Regenerates one selected strategy section from reviewer feedback.',
    ...
    execute: async ({ inputData }) => {
      ...
      if (section === 'personas') {
        const personas = await runBuyerPersona(deps.buyerPersonaAgent, {
      ...
      if (section === 'buyerJourney') {
        const buyerJourney = await runBuyerJourney(deps.buyerJourneyAgent, {
          product: strategy.product,
      ...
      const smartObjectives = await runSmartObjectives(deps.smartObjectivesAgent, {
        product: strategy.product,
  ...
  return createWorkflow({
    ...
    description: 'Regenerates one approved strategy section from specific reviewer feedback.',
    inputSchema: StrategySectionRevisionInputSchema,
    outputSchema: MarketingStrategyOutputSchema,
    ...
    planQuality: {
      ...
      issues: [
        ...
        ),
        {
          ...
          field: section,
          message: 'A strategy section changed while its dependent sections were preserved.',
          resolution: decision,
        },
      ],
      nextDecisions: [
        decision,
        ...strategy.planQuality.nextDecisions.filter((item) => item !== decision),
      ].slice(0, 8),
    },
  };
}
